...
//fonction qui permet de vider les informations de formulaire
...
form.addEventListener("submit",function(e){
    ...
    if(validation()){
        ...
        if (!exp_reg_matricule.test(matricule.value.trim())) {
            matricule.style.backgroundColor = "red";
        }
        if (!exp_reg_marq_carb.test(marque.value.trim())) {
            marque.style.backgroundColor = "red";
        }
        if (!exp_reg_marq_carb.test(carburant.value.trim())) {
            carburant.style.backgroundColor = "red";
        }
